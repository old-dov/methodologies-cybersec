# Rapport Méthodologique de Session — Audit IRP & Gestion de Crise Cybersécurité
...
winpmem.exe -o mem_dump.raw --volume_shadow_copies

# Isolation réseau logique au niveau de l'hôte via pare-feu local (remplacement du shutdown)
netsh advfirewall set allprofiles state on
netsh advfirewall firewall add rule name="IR_Isolation" dir=out action=block
```
...
2. **Durcissement des Endpoints et de l'Identité :** Déploiement d'agents EDR en mode bloquant/actif, activation de *LSA Protection* (`RunAsPPL`) sur tous les DCs et bascule des règles de détection d'exécutions de macros du mode *log-only* au mode *quarantine*.
